...
一份會主動告訴你「別太相信我剛剛給你的數字」的報告，比一份只給結論的準確得多。

</td>
</tr>
</table>

> 完整互動版在 `docs/index.html`（已移除個人持股的快照），下載後用瀏覽器直接開就能點。
...
這是這個專案最重要的一個數字，我把它放在 README 最前面。
...
## 兩個我自己犯的錯（都是 walk-forward 抓到的）

**一、`max_hold_days` 40 → 60 是過擬合。**
我看了樣本外結果才決定改這個參數，所以該區間的數字已經不乾淨。
逐年做 walk-forward 後，每一年的最佳值是 `20, 20, 60, 20, 60, 20, 80, 80, 20, 40`——
**沒有穩定解**，而且 60 在十年尺度上是五個候選裡**最差**的（+374.7%，20 天是 +467.9%）。

我**刻意沒有把它改成 20**。那會是同一個錯誤再犯一次：又一次看了結果才選參數。
現在它留在 60，註解裡寫明它是怎麼被選出來的，以及為什麼不動它。

**二、自建市場指數跑出 +4.3 億%。**
用觀察池的成分股自建等權重指數當基準，結果是天文數字。
原因是對一個有選擇偏誤的池子做每日再平衡——等於每天賣掉跌的、買進漲的，
而池子本身又是用期末資訊挑的。基準必須用**完整股價歷史**重建，不能用過濾後的 panel。
同一類錯誤還有一次：拿平均報酬去複利。偏態分布下這會算出荒謬的數字，必須用幾何平均。
